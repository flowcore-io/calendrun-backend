...
- Provide workspace scope and begin with `repo:calendrun-backend` tags; iterate until the tool reports `decision: "SUFFICIENT"`.
...
- Respect the tool's safety rails—if you see `invocationLimitReached: true`, stop rerunning the tool and document the uncovered gaps instead. Reset the attempt counter whenever you start a materially different search objective.
...
Repository: calendrun-backend
WorkspaceId: 60c10ca2-4115-4c1a-b6d7-04ac39fd3938
Workspace: Flowcore
Workspace Fragment Types: knowledge, recipe, solution, template, architectural decision, commands, feature request, infrastructure, instruction set, issue, llm persona, llm rules, outage investigation, plan, prd, research, violation exception
